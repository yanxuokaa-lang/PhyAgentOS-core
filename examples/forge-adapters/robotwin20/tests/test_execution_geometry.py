...
import numpy as np
import robotwin_blocks_ranking_adapter as task_adapter_module
import robotwin_route_input_worker as worker


def test_execution_geometry_never_reads_task_targets(tmp_path, monkeypatch):
    (tmp_path / "envs").mkdir()
    (tmp_path / "envs/blocks_ranking_rgb.py").write_text("# test task\n")
    ...
    monkeypatch.setattr(worker, "load_runtime_profile", lambda _: {
        "task_name": "blocks_ranking_rgb", "task_config": "test", "embodiment": "test", "seed": 0})
    monkeypatch.setattr(task_adapter_module, "_half_extents", lambda _: [.02]*3)
