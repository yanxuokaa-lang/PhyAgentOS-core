...
`AgentLoopNodeExecutor` 从空历史启动，只投影当前节点以及直接前驱/显式 evidence 的字段目录、
计数、引用和小型身份摘要；大型几何 payload 留在 Coordinator。`manipulation.prepare` 通过
`record_id` 与精确字段路径选择 `grasp.propose` 的完整候选数组，由 Coordinator 解析后按冻结
Consumer schema 校验，同时不会重新注入无关的全局执行历史。节点回合只暴露当前节点所需的 context、ready、select 和受治理的 Query/
...
`required_evidence` 同时选中的 discovery Query 请求参数与终态结果中选择并结构化组合参数；非刷新
Query 请求和响应中的任何显式 scene revision 都必须与当前规划场景及彼此一致。冻结 policy 声明
`refreshes_scene` 的 Tool 可在请求中指明源场景，但响应必须指明当前规划场景。`forge_plan_ready` 只为当前候选
投影 task-bound 冻结 schema；实时 `forge_tool_context` 继续负责 readiness，并仅作为旧绑定缺少
冻结 schema 时的兼容回退。Agent 不能创造观测、度量几何、标定、freshness 或执行事实。
`AgentComposedDispatch` 在创建 DecisionTrace 和调用 Gateway 前按冻结的消费者 schema 校验最终对象，失败时不创建 Tool record、不调用 Gateway。
...
若 recovery proposer 失败，Coordinator 先进入既有 `awaiting_replan`；如果预算耗尽等原因使该
迁移不可用，则使用既有 `failed` 终态持久化 recovery 失败。runner 不得返回 `blocked` 却让权威
任务继续保持 `executing` 并反复处理同一失败 settlement。
